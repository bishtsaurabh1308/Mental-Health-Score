...
(() => {
  ...
  // Reset the form for a fresh prediction while retaining the unit and score scales.
  function resetPredictionForm() {
    form.reset();
    stressHiddenInput.value = "";
    segGroup.querySelectorAll(".seg-btn").forEach((btn) => btn.classList.remove("active"));
    clearAllErrors();
    showState("idle");
  }

  resetBtn.addEventListener("click", resetPredictionForm);
  errorRetryBtn.addEventListener("click", resetPredictionForm);
})();
