...
(() => {
  ...
  // StudentData (Pydantic) uses exact-case field names — Pydantic is
  // case-sensitive with no aliases, so lowercase form field names must be
  // remapped before the request goes out.
  const FIELD_MAP = {
    age: "Age",
    gender: "Gender",
    country: "Country",
  ...
  const REVERSE_FIELD_MAP = Object.fromEntries(
    Object.entries(FIELD_MAP).map(([htmlId, apiField]) => [apiField, htmlId])
  );
  ...
  function applyServerValidationErrors(detail) {
    ...
    detail.forEach((err) => {
      ...
      const htmlId = field ? (REVERSE_FIELD_MAP[field] || field) : null;
      const input = htmlId ? document.getElementById(htmlId) : null;
      const target = htmlId === "stress_level" ? stressHiddenInput : input;
      if (target) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      renderError(
        ...
        `Couldn't connect to ${API_URL}. Make sure the backend is running (uvicorn main:app --reload) and reachable from this page.`
      );
    } finally {
      setSubmitting(false);
    }
